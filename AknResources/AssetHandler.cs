...
namespace AknResources {
    public static class AssetHandler {
        ...
        private static bool ShouldExport(HandlingContext ctx, string name, string extension, out string fullName) {
            fullName = Path.Combine(ctx.Directory, name + extension);
            var result = !File.Exists(fullName) || File.GetLastWriteTimeUtc(fullName) < ctx.BundleTime;
            Directory.CreateDirectory(ctx.Directory);

            if (ctx.Verbose) {
                Log.Information(result ? $"Export: {fullName}" : $"Skip: {fullName}");
            }
            return result;
        }

        private static void HandleTexture2D(Texture2D obj, HandlingContext ctx) {
            if (ShouldExport(ctx, obj.m_Name, ".png", out var fullName)) {
        ...
        private static void HandleAudioClip(AudioClip obj, HandlingContext ctx) {
            ...
            if (ShouldExport(ctx, obj.m_Name, extension, out var fullName)) {
                byte[] data;
                if (convert) {
        ...
        private static void HandleTextAsset(TextAsset obj, HandlingContext ctx) {
            if (ShouldExport(ctx, obj.m_Name, ".txt", out var fullName)) {
